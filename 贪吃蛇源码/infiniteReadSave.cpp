...
int main() {
	...
	while (1) {
		...
		//              2.蛇的长度刚好是一个数的倍数
		if (!snake->poison || ((snake->length) % 1 == 0 && snake->length != prelength) || createMoreFood % snake->poisonRate == 0) {
...
Snake* readSave() {
	IMAGE save;
	loadimage(&save,"d:\\snakefile\\resource\\InfiniteResource\\save.jpg",630,420);
	putimage(0,0,&save);
	//==========================================
	...
	//==========================================
	FILE *fp;
	fopen_s(&fp,"d:\\snakefile\\resource\\InfiniteResource\\snakedata.txt","r");
	fscanf_s(fp, "%d", &(p->direction));
	fscanf_s(fp, "%d", &(p->length));
	fscanf_s(fp, "%d", &(p->score));
	fscanf_s(fp, "%d", &(p->speed));
	fscanf_s(fp, "%d", &(p->food));
	fscanf_s(fp, "%d", &(p->poison));
	fscanf_s(fp, "%d", &(p->star));
	fscanf_s(fp, "%d", &(p->foodRate));
	fscanf_s(fp, "%d", &(p->poisonRate));
	fscanf_s(fp, "%d", &(p->starRate));
	fscanf_s(fp, "%d", &(p->foodNum));
	fscanf_s(fp, "%d", &(p->poisonNum));
	fclose(fp);
	//===========================================
	if (p->direction == 1) {
		p->dx = 0;
	...
	//==========================================
	fopen_s(&fp, "d:\\snakefile\\resource\\InfiniteResource\\snake.txt", "r");
	int k = 0;
	SnakeBody *sp = NULL;
	while (k <= p->length) {
	...
	fclose(fp);
	//==========================================
	fopen_s(&fp, "d:\\snakefile\\resource\\InfiniteResource\\poison.txt", "r");
	k = 0;
	Poison *poison = NULL;
	while (k < p->poisonNum) {
	...
	fclose(fp);
	//==========================================
	fopen_s(&fp, "d:\\snakefile\\resource\\InfiniteResource\\food.txt", "r");
	k = 0;
	Food *food = NULL;
	while (k < p->foodNum) {
	...
	//===========================================
	if (p->star) {
		fopen_s(&fp, "d:\\snakefile\\resource\\InfiniteResource\\star.txt", "r");
